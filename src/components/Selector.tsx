...
import type { TryConfig, TryEntry, ScoredEntry, SelectorResult } from "../types.js";
...
export function Selector({ config, onResult }: SelectorProps) {
    ...
    (input, key) => {
      ...
      if (key.downArrow || (key.ctrl && input === "n") || input === "j") {
        setSelectedIndex((prev) =>
          Math.min(prev + 1, scoredEntries.length - 1)
        );
      ...
      // Create new directory
        ...
        // Already handled above for navigation, use 'n' without ctrl
      } else if (input === "n" && !key.ctrl && query === "") {
        // 'n' with empty query creates new
        // Note: This conflicts with typing 'n' in search
        // So we only trigger on ctrl+n when query has content
      }
      // Actually, let's use ctrl+c for creating - that conflicts with exit
      // Let's keep it simple: Enter on empty search creates, or we show hint
      // Delete entry
  ...
  const showCreateHint =
    query.trim() && scoredEntries.length === 0;
    ...
    <Box flexDirection="column">
      ...
      {showCreateHint && (
        <Box marginTop={1} paddingLeft={2}>
          <Text color="green">
            Press Enter to create: {createDirName(query)}
          </Text>
        </Box>
      )}
